...
X = [dataTest(:, 1) cos(dataTest(:, 1)) sin(dataTest(:, 1))];
y = dataTest(:, 4);
m = length(y);
...
for i = 1:m
    ThetaTEST (i)= theta(1)+...
        ...
        theta(3)*(cos(dataTest(i, 1)) -mu(2))/sigma(2)+...
        theta(4)*(sin(dataTest(i, 1)) -mu(3))/sigma(3);
...
%         theta(5)*(dataTest(i, 4)-mu(4))/sigma(4)]; % Enter your price formula here
    ...
    ThetaGT (i)= theta(1)+...
        ...
        theta(3)*(cos(data(i, 1)) -mu(2))/sigma(2)+...
        theta(4)*(sin(data(i, 1)) -mu(3))/sigma(3);
